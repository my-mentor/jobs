  ...
  all_teachers.each do |teacher_hash|
    teacher_hash["skills"].each do |skill|
      student_teacher_list << teacher_hash if skill["fields"] == fields && skill["levels"] == levels
    end
  end
  p "This is the list of professor you match with :"
  student_teacher_list.each { |list| p "#{list['id']} - #{list['firstname']} #{list['lastname']}" }
  ...
  student_teacher_list.each_with_index do |twin, index|
     choice_array << twin if index == choice_teacher
  end
